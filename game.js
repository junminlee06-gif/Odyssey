...
const entities=[{id:'poster',title:'영웅 포스터',x:340,kind:'object'},{id:'list',title:'귀환병 명단',x:780,kind:'object'},{id:'prop',title:'선전관',x:1240,kind:'npc'},{id:'cargo',title:'화물칸 문',x:1780,kind:'object'},{id:'survivor',title:'트로이 생존자',x:2380,kind:'npc'},{id:'ticket',title:'검표 구역',x:3020,kind:'object'},{id:'inspector',title:'검문관',x:3480,kind:'npc'},{id:'gate',title:'검문 게이트',x:4040,kind:'gate'}];
...
function drawImage(image,x,y,w,h,flip=false){if(!image.complete||image.naturalWidth===0)return false;ctx.save();if(flip){ctx.scale(-1,1);ctx.drawImage(image,-Math.round(x+w),Math.round(y),Math.round(w),Math.round(h));}else ctx.drawImage(image,Math.round(x),Math.round(y),Math.round(w),Math.round(h));ctx.restore();return true;}
...
function drawFallbackStation(){rect(0,0,VIEW_W,VIEW_H,'#171719');rect(0,0,VIEW_W,120,'#2b3134');rect(0,214,VIEW_W,8,'#11100e');for(let x=-80;x<VIEW_W+160;x+=160){line(x,230,x+92,58,'#5c564c');line(x+92,58,x+184,230,'#5c564c');rect(x+88,204,12,162,'#070605');}}
function drawBackground(){rect(0,0,VIEW_W,VIEW_H,C.void);const off=-Math.floor(cameraX*.12)%768;if(!drawTiled(assets.station,off-768,0,1536,432,VIEW_W+1536))drawFallbackStation();rect(0,0,VIEW_W,VIEW_H,'rgba(0,0,0,.08)');}
...
function drawTrainSteam(){const period=4300,active=1650,local=currentTime%period;if(local>active)return;const baseT=local/active,off=-Math.floor(cameraX*.62)%936;for(let base=off-936;base<VIEW_W+936;base+=936){[base+82,base+402,base+706].forEach((ventX,vi)=>{for(let i=0;i<5;i++){const t=baseT-i*.12-vi*.04;if(t<=0||t>1)continue;const drift=t*(36+i*7),rise=t*(64+i*10),size=8+t*18+i*2;steamPuff(ventX-drift+i*7,270-rise-i*4,size,i%2?C.steamA:C.steamB);}});}}
function drawTrain(){const y=258,off=-Math.floor(cameraX*.62)%936;if(!drawTiled(assets.train,off-936,y,936,110,VIEW_W+936)){rect(0,y+12,VIEW_W,84,'#332012');rect(0,y+18,VIEW_W,2,C.gold0);rect(0,y+78,VIEW_W,2,C.gold0);}drawTrainSteam();}
...
function fallbackPoster(x,y){rect(x,y,56,70,C.black);rect(x+5,y+5,46,60,C.paper);rect(x+18,y+12,20,22,'#6b3326');rect(x+36,y+53,12,10,C.red);}
function fallbackList(x,y){rect(x,y,48,82,C.black);rect(x+5,y+5,38,72,'#3a2515');rect(x+12,y+16,24,4,C.gold1);rect(x+12,y+28,20,3,C.gold0);rect(x+12,y+50,22,3,C.gold0);}
function fallbackCargo(x,y){rect(x,y+8,132,60,C.black);rect(x+7,y+4,116,58,'#4c2a16');rect(x+26,y+23,68,4,C.gold0);rect(x+37,y+38,54,4,C.gold1);}
function fallbackCheckpoint(x,y){rect(x,y+14,146,104,C.black);rect(x+8,y+22,130,88,'#463019');rect(x+16,y+38,110,6,C.gold1);line(x+132,y+18,x+184,y+68,C.gold1);}
function drawForegroundObjects(){const visible=wx=>sx(wx)>-240&&sx(wx)<VIEW_W+240;if(visible(340)){const x=sx(290),y=278;if(!drawImage(assets.poster,x,y,56,70))fallbackPoster(x,y);}if(visible(780)){const x=sx(740),y=270;if(!drawImage(assets.listBoard,x,y,48,82))fallbackList(x,y);}if(visible(1780)){const x=sx(1680),y=276;if(!drawImage(assets.cargo,x,y,132,72))fallbackCargo(x,y);}if(visible(3020)){const x=sx(2980),y=270;if(!drawImage(assets.listBoard,x,y,48,82))fallbackList(x,y);}if(visible(4040)){const x=sx(4040),y=244;if(!drawImage(assets.checkpoint,x,y,184,118))fallbackCheckpoint(x,y);}}
function drawNpc(entity){const x=sx(entity.x);if(x<-160||x>VIEW_W+160)return;const frame=Math.floor((currentTime+entity.x)/720)%2,bob=frame?1:0;let image=frame?assets.propB:assets.prop;if(entity.id==='survivor')image=frame?assets.survivorB:assets.survivor;if(entity.id==='inspector')image=frame?assets.inspectorB:assets.inspector;rect(x-20,GROUND_Y-3,44,5,'rgba(0,0,0,.55)');drawImage(image,x-21,GROUND_Y-58-bob,42,58);}
...
function drawAtmosphere(){const phase=Math.floor(currentTime/45);for(let i=0;i<44;i++){const x=(i*87+phase*3)%VIEW_W,y=(i*37+phase*7)%VIEW_H;rect(x,y,1,5,'rgba(215,179,93,.07)');}for(let y=0;y<VIEW_H;y+=3)rect(0,y,VIEW_W,1,'rgba(0,0,0,.04)');}
function drawScene(){drawBackground();drawTrain();drawForegroundObjects();drawPlatform();drawParticles();entities.forEach(e=>{if(e.kind==='npc')drawNpc(e);});drawHero();entities.forEach(e=>{const x=sx(e.x);if(x<-160||x>VIEW_W+160)return;drawGlyph(x,GROUND_Y-115,near&&near.id===e.id);});drawAtmosphere();}
