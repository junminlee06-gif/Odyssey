...
function drawForegroundObjects(){trainScratches();const visible=wx=>sx(wx)>-300&&sx(wx)<VIEW_W+300;if(visible(340)){const x=sx(292),y=263;mountShadow(x,y,66,80);if(!drawImage(assets.poster,x,y,66,80))fallbackPoster(x,y);}if(visible(780)){const x=sx(720),y=236;mountShadow(x,y,72,104);if(!drawImage(assets.listBoard,x,y,72,104))fallbackList(x,y);}if(visible(1780)){const x=sx(1650),y=238;mountShadow(x,y,178,104,'gate');if(!drawImage(assets.cargo,x,y,178,104))fallbackCargo(x,y);}if(visible(3020)){const x=sx(2954),y=236;mountShadow(x,y,72,104);if(!drawImage(assets.listBoard,x,y,72,104))fallbackList(x,y);}if(visible(4040)){const x=sx(4020),y=204;mountShadow(x,y,220,148,'gate');if(!drawImage(assets.checkpoint,x,y,220,148))fallbackCheckpoint(x,y);}}
