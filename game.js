...
const entities=[{id:'poster',title:'영웅 포스터',x:340,kind:'object',glyphY:292},{id:'list',title:'귀환병 명단',x:780,kind:'object',glyphY:284},{id:'prop',title:'선전관',x:1240,kind:'npc',glyphY:250},{id:'cargo',title:'화물칸 문',x:1780,kind:'object',glyphY:292},{id:'survivor',title:'트로이 생존자',x:2380,kind:'npc',glyphY:250},{id:'ticket',title:'검표 구역',x:3020,kind:'object',glyphY:284},{id:'inspector',title:'검문관',x:3480,kind:'npc',glyphY:250},{id:'gate',title:'검문 게이트',x:4040,kind:'gate',glyphY:264}];
...
function drawBackground(){rect(0,0,VIEW_W,VIEW_H,C.void);const off=-Math.floor(cameraX*.12)%768;if(!drawTiled(assets.station,off-768,0,1536,432,VIEW_W+1536))drawFallbackStation();rect(0,0,VIEW_W,VIEW_H,'rgba(0,0,0,.045)');}
...
function drawTrainDepth(y){rect(0,y+92,VIEW_W,8,'rgba(0,0,0,.52)');for(let x=-Math.floor(cameraX*.62)%96-96;x<VIEW_W+120;x+=96){rect(x,y+96,54,4,'rgba(120,83,38,.45)');rect(x+10,y+103,34,2,'rgba(0,0,0,.55)');}}
function drawTrain(){const y=252,off=-Math.floor(cameraX*.62)%936;if(!drawTiled(assets.train,off-936,y,936,110,VIEW_W+936)){rect(0,y+12,VIEW_W,84,'#332012');rect(0,y+18,VIEW_W,2,C.gold0);rect(0,y+78,VIEW_W,2,C.gold0);}drawTrainDepth(y);drawTrainSteam();}
...
function trainScratches(){const off=-Math.floor(cameraX*.62)%120;for(let x=off-120;x<VIEW_W+120;x+=120){rect(x+16,293,42,2,'rgba(215,179,93,.20)');rect(x+70,318,55,2,'rgba(115,76,34,.34)');rect(x+46,276,1,50,'rgba(0,0,0,.22)');rect(x+98,282,2,2,'rgba(217,179,93,.28)');rect(x+104,282,2,2,'rgba(217,179,93,.20)');}}
function drawForegroundObjects(){trainScratches();const visible=wx=>sx(wx)>-260&&sx(wx)<VIEW_W+260;if(visible(340)){const x=sx(286),y=286;mountShadow(x,y,58,64);if(!drawImage(assets.poster,x,y,58,64))fallbackPoster(x,y);}if(visible(780)){const x=sx(735),y=274;mountShadow(x,y,52,78);if(!drawImage(assets.listBoard,x,y,52,78))fallbackList(x,y);}if(visible(1780)){const x=sx(1672),y=279;mountShadow(x,y,146,70,'gate');if(!drawImage(assets.cargo,x,y,146,70))fallbackCargo(x,y);}if(visible(3020)){const x=sx(2974),y=274;mountShadow(x,y,52,78);if(!drawImage(assets.listBoard,x,y,52,78))fallbackList(x,y);}if(visible(4040)){const x=sx(4034),y=238;mountShadow(x,y,194,116,'gate');if(!drawImage(assets.checkpoint,x,y,194,116))fallbackCheckpoint(x,y);}}
...
function drawScene(){drawBackground();drawTrain();drawForegroundObjects();drawPlatform();drawParticles();entities.forEach(e=>{if(e.kind==='npc')drawNpc(e);});drawHero();entities.forEach(e=>{const x=sx(e.x);if(x<-160||x>VIEW_W+160)return;drawGlyph(x,e.glyphY||GROUND_Y-112,near&&near.id===e.id);});drawAtmosphere();}
