  ...
  return <main className="game">
    ...
    <section className="viewport" onPointerDown={tapMove}>
      <div className="world" style={{ transform: `translateX(${-camera}px)` }}>
        <div className="roof" />
        <div className="train">17번 승강장 · 이타카행 연결편</div>
        <div className="refugees">피난민 줄</div>
        {entities.map(e => <button className={`marker ${e.kind}`} style={{ left: e.x }} key={e.id} onClick={() => interact(e)}>{e.title}</button>)}
